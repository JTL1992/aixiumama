...
public class NetUtil {
	...
	public static boolean urlDownloadToFile(Context context, String url,
		...
		try {
			...
			if(path!=null){
				...
				file=new File(Environment.getExternalStorageDirectory()+"/cutesdk/", ""+System.currentTimeMillis());
			}
	...
	public static Object get(RequestVo vo) throws Exception {
        ...
        if(AppSharedPref.newInstance(vo.context).getToken() != null) {
            try {
                method.addHeader("Authorization", "token " + AppSharedPref.newInstance(vo.context).getToken());
                LogUtil.v("发送的Header", "token " + AppSharedPref.newInstance(vo.context).getToken());
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
